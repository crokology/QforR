...
Another way of highlighting factor differences is to compute the pairwise z-score differences between two factors. This operation is already being done and the differences are storred in the background, we just need to call it with the right command. 

```{r}
results$qdc[order(results$qdc$f1_f2, decreasing = T), ]

results$qdc[order(results$qdc$f1_f3, decreasing = T), ]

results$qdc[order(results$qdc$f2_f3, decreasing = T), ]
```

Apart from the z-score differences, we also see whether or not these differences are significant or not.
